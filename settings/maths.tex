...
\newcommand{\spacefortrans}[1]{
	\begin{axis}[
		...
		\draw[very thick, xgreen, fill=xgreen, fill opacity=0.25] (-2,1) ellipse (1 and 1.5);
		\draw[very thick, densely dotted, xpurple] (-5,-2) -- (5,-1);
		\draw[very thick, densely dotted, xpurple] (-5,-3) -- (5,-2);
		\draw[very thick, xblue, fill=xblue, fill opacity=0.25] (2,2) rectangle (4,3);
	\end{axis}
}

\newcommand{\drawtrans}[4]{
	\begin{tikzpicture}
		\spacefortrans{name=beforetrans}
