...
\newcommand{\fromtranstomat}[6]{
	\begin{tikzpicture}
		...
		\node[below of=e1col, yshift=-1.25cm] (e1map) {$\colvec{#3;#4}$};
		\node[below of=e2col, yshift=-1.25cm] (e2map) {$\colvec{#5;#6}$};
		\draw[|->] (e1col.south) -- (e1map.north);
		\draw[|->] (e2col.south) -- (e2map.north);
		\draw[dashed] (-3,-2) -- ++(6,0);
		\node[fill=white] (T) at (0,-2) {$#1$};
		\node (A) at (0, -5.5) {$\begin{bmatrix}#3&#5\\#4&#6\end{bmatrix}$};
		\draw[->, xpurple] (e1map.south) to[out=270, in=90] ($(A.north)-(0.3,0)$);
		\draw[->, xorange] (e2map.south) to[out=270, in=90] ($(A.north)+(0.3,0)$);
		\node[below of=A] {$#2$};
	\end{tikzpicture}
}
