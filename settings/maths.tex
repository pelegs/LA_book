...
% Geometric objects notations
\newcommand{\pnt}[1]{\mathbf{#1}}
\newcommand{\lin}[1]{\overline{\mathbf{#1}}}
\newcommand{\tria}[1]{\triangle \mathbf{#1}}
...
% Specific often-used vectors
\newcommand{\vecA}{\textcolor{xred}{\vvec{\mathbf{a}}}}
\newcommand{\vecB}{\textcolor{xblue}{\vvec{\mathbf{b}}}}
\newcommand{\vecC}{\textcolor{xorange}{\vvec{\mathbf{c}}}}
...
\newcommand{\GenericColVecXY}[1]{\colvec{#1_{x};#1_{y}}}
\newcommand{\GenericColVecXYZ}[1]{\colvec{#1_{x};#1_{y};#1_{z}}}

% Colored vectors
\colorlet{xcomp}{xred}
\colorlet{ycomp}{xblue}
\colorlet{zcomp}{xgreen}
\setlength{\fboxsep}{10pt}
\newcommand{\ClrColVecXY}[2]{\colvec{\colorbox{xcomp!20}{#1};\colorbox{ycomp!20}{#2}}}
\newcommand{\ClrColVecXYZ}[3]{\colvec{\colorbox{xcomp!20}{#1};\colorbox{ycomp!20}{#2};\colorbox{zcomp!20}{#3}}}
\newcommand{\GenericClrColVecXY}[1]{\ClrColVecXY{$#1_{x}$}{$#1_{y}$}}
...
\newcommand{\ebcol}[2]{\colorbox{#1}{$\eb{#2}$}}
\newcommand{\ebcolxyz}[1]{\ebcol{xcol#1!25}{#1}}  % TODO: This should be re-written as cases with letters vs. integers, etc.
\newcommand{\ebx}{\colorbox{xred!25}{$\hat{x}$}}
\newcommand{\eby}{\colorbox{xblue!25}{$\hat{y}$}}
\newcommand{\ebz}{\colorbox{xgreen!25}{$\hat{z}$}}
...
% LTs with "names"
\newcommand{\Rotation}[2]{\text{Rot}_{#1}\left(#2\right)}
\newcommand{\RotDeg}[2]{\Rotation{\ang{#1}}{#2}}
\newcommand{\Reflection}[1]{\text{Ref}_{#1}}
\newcommand{\ReflectionFunc}[2]{\Reflection{#1}\left(#2\right)}
...
\newcommand{\clrgenericxyz}[2]{
	\colvec{\LCa{#1_{1}};\LCb{#1_{2}};\LCc{#1_{3}}}
}
...
\newcommand{\fromtranstomat}[6]{
	% NOTE: this macro needs some improvements
	\begin{tikzpicture}[ampersand replacement=\&]
		\tikzset{mapstoarrow/.style={|->, thick, xpurple}}

		\draw[thick, dotted, black!25] (-2,-2.25) -- ++(4,0) node[midway, fill=white, text=xpurple] (T) {$#1$};

		\node (e1) at (-1.,0) {$\ebx$};
		\node (e2) at (+1.,0) {$\eby$};
		\node[below of=e1] (e1col) {$\colvec{1;0}$};
		\node[below of=e2] (e2col) {$\colvec{0;1}$};
		\node[below of=e1col, yshift=-1.2cm] (Te1) {$#1\left(\ebx\right)$};
		\node[below of=e2col, yshift=-1.2cm] (Te2) {$#1\left(\eby\right)$};
		...
		\begin{scope}[on background layer]
			\filldraw[xred, rounded corners, opacity=0.25] (A-1-1.north west) rectangle (A-2-1.south east);
			\filldraw[xblue, rounded corners, opacity=0.25] (A-1-2.north west) rectangle (A-2-2.south east);
		\end{scope}

		\draw[-stealth, thick, xred] (e1map.south) to[out=270, in=90] (A-1-1.north);
		\draw[-stealth, thick, xblue] (e2map.south) to[out=270, in=90] (A-1-2.north);
		\node[below of=A, text=xpurple] {$#2$};
	\end{tikzpicture}
}
