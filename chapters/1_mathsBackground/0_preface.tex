	...
	\begin{center}
		\begin{tikzpicture}
			\begin{axis}[
					...
					axis line style={stealth-stealth, very thick, opacity=0.25},
					grid=major,
				...
				\coordinate (v) at ($-1*(b1)+2*(b2)$);
				\draw[vector, xorange] (0,0) -- (v) node[pos=1.1] {$\vec{v}$};

				\draw[vector, xred, dashed] (0,0) -- (1,0);
				\draw[vector, xred, dashed] (1,0) -- (2,0);
				\draw[vector, xred, dashed] (2,0) -- (3,0);

				\draw[vector, xblue, dashed] (3,0) -- (3,1);
				\draw[vector, xblue, dashed] (3,1) -- (3,2);
				\draw[vector, xblue, dashed] (3,2) -- (3,3);
				\draw[vector, xblue, dashed] (3,3) -- (3,4);
				\draw[vector, xblue, dashed] (3,4) -- (3,5);
			\end{axis}
			\begin{axis}[
					xyplane,
					anchor=origin,
				...
				\draw[vector, xorange] (0,0) -- (3,5) node[pos=1.1] {$\vec{v}$};

				\draw[vector, xgreen, dashed] (0,0) -- (1,-1);

				\draw[vector, xpurple, dashed] (1,-1) -- (2,2);
				\draw[vector, xpurple, dashed] (2,2) -- (3,5);
			\end{axis}
		\end{tikzpicture}
	\end{center}
	\caption{}
...
And now I will test my \enquote{synonyms} command: here I'm talking about a new topic and I introduce a conecept, say \synonyms{linear transformations}{linear functions, linear maps}{xgreen}. It should be showing some synonyms in the page margins. Does it work? I hope so! Here's a classic margin note: bla bla\marginnote{\tcbox[
		enhanced,
		remember as=synonymsbox,
		boxrule=0pt,
		frame hidden,
		borderline west={8pt}{0pt}{xpurple},
		colback=xpurple!25,
		sharp corners,
		left=10pt,
		right=3pt,
		top=3pt,
		bottom=3pt,
	] {test test test, la la la la.}} bla.
