...
% 	\end{center}
	...
	\begin{center}
		...
		\begin{tikzpicture}[tdplot_main_coords]
			\draw[vector, xred] (-6,0,-2) -- ++(1,0,0) node[pos=1.2] {$\ebx$};
			\draw[vector, xblue] (-6,0,-2) -- ++(0,1,0) node[pos=1.3] {$\eby$};
			\draw[vector, xgreen] (-6,0,-2) -- ++(0,0,1) node[pos=1.2] {$\ebz$};
			\tdplotsetrotatedcoords{50}{140}{60}
			\begin{scope}[tdplot_rotated_coords]
				\begin{scope}[canvas is xy plane at z=0]
					\fill[xpurple, fill opacity=0.25] (-3,-3) rectangle (3,3); 
					\draw[step=1.0, dashed, very thick, xpurple, opacity=0.1] (-3,-3) grid (3,3);
					\pgflowlevelsynccm
				\end{scope} 
			\end{scope}
			\draw[vector, xorange] (0,0,0) -- (3,1,2) node[pos=1.1] {$\vec{r}$};
			\draw[vector, xdarkgreen] (-3,-1,2) -- (0,0,0); % should be d-2n<d|n>
		\end{tikzpicture}
	\end{center}
\end{motivation}
